...
if ($id == '') {
	...
	$result = run_query("SELECT pageid, title FROM ja08prat_pages");
...
// If action is update
	...
	// Form the query
	$sql = "UPDATE ja08prat_pages SET title = '$title', content = '$content', parent = '$parent' WHERE pageid='$id'";
	...
	// Echo feedback
	echo '
		<p><a href="index.php?pageid='.$id.'">'.$id.'</a> was updated from ja08prat_pages</p>';
}
...
// If the id is given but action is not update
	...
	$result = run_query("SELECT * FROM ja08prat_pages WHERE pageid='$id'");
