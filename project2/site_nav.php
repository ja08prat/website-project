...
function echo_nav($short_title, $current_page) {
	...
	$pages_level1 = run_query("SELECT pageid, title FROM ja08prat_pages WHERE parent = 'home'");
	...
	while ($page_lv1 = $pages_level1->fetch_assoc()) {
		...
		$pages_level2 = run_query("SELECT pageid, title FROM ja08prat_pages WHERE parent = '".$pageid."'");
